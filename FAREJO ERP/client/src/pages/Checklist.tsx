...
type Task = { id: number; tenantId: number; categoryId: number | null; name: string; status: "pending" | "in_progress" | "done"; priority: "urgent" | "week" | "later"; responsible: string | null; position: number | null; alertCampaignId: number | null; alertDaysBefore: number | null; recurrence: "once" | "daily" | "weekly" | null; imageUrl: string | null; link: string | null; recurringDays: string | null; createdAt: Date; updatedAt: Date; };
...
export default function Checklist() {
  ...
  const [editForm, setEditForm] = useState({ name: "", priority: "week" as Task["priority"], responsible: "", recurrence: "once" as "once" | "daily" | "weekly", imageFile: null as File | null, imageUrl: null as string | null, link: "", recurringDays: [] as number[] });
  ...
  const handleAddTask = async () => {
    ...
    createTaskMutation.mutate({
      ...
      responsible: newTaskResponsible.trim() || undefined,
    ...
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      {!isReadOnly && (
        <div className="bg-card border border-border rounded-xl p-3 mb-4 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Input className="w-36 text-xs" placeholder="Responsável" value={newTaskResponsible} onChange={(e) => setNewTaskResponsible(e.target.value)} />
      ...
      {view === "list" ? (
        <ListView
          ...
          onEdit={(t: Task) => {
            setEditingTask(t);
            setEditForm({ name: t.name, priority: t.priority, responsible: t.responsible ?? "", recurrence: (t.recurrence ?? "once") as "once" | "daily" | "weekly", imageFile: null, imageUrl: t.imageUrl ?? null, link: t.link ?? "", recurringDays: t.recurringDays ? JSON.parse(t.recurringDays) : [] });
          }}
        />
      ) : (
        <KanbanView
              ...
              {(["urgent", "week", "later"] as const).map((p) => (
                <button
                  ...
                  className={`flex-1 py-1.5 text-xs font-semibold rounded-lg border transition-all ${
                    quickAddPriority === p ? "bg-[#C9A227] text-black border-[#C9A227]" : "border-border text-muted-foreground hover:border-[#C9A227]/50"
                  }`}
                >
                  {PRIORITY_LABELS[p]}
                </button>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Input className="w-36 text-xs" placeholder="Responsável" value={editForm.responsible} onChange={(e) => setEditForm({ ...editForm, responsible: e.target.value })} />
            </div>
              ...
              <div className="flex gap-1 flex-wrap">
                {["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"].map((d, i) => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded text-xs border transition-colors ${
                      editForm.recurringDays.includes(i)
                        ? "bg-[#C9A227] text-black border-[#C9A227] font-bold"
                        : "border-border text-muted-foreground hover:border-[#C9A227]/50"
                    }`}
                  >{d}</button>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                updateTaskMutation.mutate({
                  ...
                  responsible: editForm.responsible.trim() || undefined,
      ...
      <div className="p-3 space-y-2 flex-1" style={{ minHeight: "8rem" }}>
        ...
        {tasks.length === 0 && (
          <div className={`flex items-center justify-center h-16 rounded-lg border-2 border-dashed transition-colors ${
            isOver ? "border-[#C9A227]/50 text-[#C9A227]" : "border-border text-muted-foreground"
          }`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
            task.status === "pending" ? "bg-gray-500/10 text-gray-500" :
